...
# II.8. Vista de Detalle del Producto
...
# II.9. Formulario de Solicitud de Pedido (Manejo Directo en la Vista)
def solicitar_pedido(request, producto_id=None):
    ...
    if request.method == 'POST':
        # Validación mínima: el nombre del cliente y la descripción son obligatorios
        ...
        if not nombre_cliente or not descripcion:
            ...
            return redirect(request.path_info) # Recarga la página con el error

        try:
            # 1. Crear el objeto Pedido con datos del POST
            # El campo 'producto_referencia' se obtiene del campo oculto en el template
            producto_ref_id = request.POST.get('producto_referencia')
            producto_ref = None
            if producto_ref_id:
                 # Si viene de un catálogo, usa el producto de referencia
                producto_ref = Producto.objects.get(pk=producto_ref_id)
            
            # Si no viene de un catálogo, usa el producto_id pasado por URL
            elif producto_id:
            ...
            nuevo_pedido = Pedido.objects.create(
                ...
                # Asignaciones automáticas [II.9. Al enviar]
                estado='SOLICITADO',
                estado_pago='PENDIENTE',
                plataforma_origen='SITIO_WEB' 
            )

            # 2. Manejo manual de imágenes de referencia (multiple file upload)
            ...
            # 3. Muestra la URL de seguimiento al cliente
